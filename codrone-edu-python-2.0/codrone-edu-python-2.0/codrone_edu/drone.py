...
def formatFirmareVersion(version):
    if version == None:
        return ""
...
class Drone:
    ...
    def __init__(self, flagCheckBackground=True, flagShowErrorMessage=False, flagShowLogMessage=False,
        ...
        self.monitorData = []
        self.information_data = [0, 0, 0, 0]  # Drone model, drone firmware, controller model, controller firmware
        self.address_data = [0, 0] # Drone address, controller address
    ...
    def get_temperature(self, unit="C"):
        ...
        return self.get_drone_temperature()
    ...
    def get_flow_velocity_y(self, unit="cm"):
        ...
        :return: distance in cm
        """
        return self.convert_meter(self.get_flow_data()[2], "cm")
    ...
    def get_trim_data(self, delay=0.7):
    ...
    def update_count_data(self, count):
        self.count_data[0] = count.timeFlight
        self.count_data[1] = count.countTakeOff
        self.count_data[2] = count.countLanding
        self.count_data[3] = count.countAccident
    ...
    def update_information(self, information):
        ...
        if deviceType == DeviceType.Drone:
            self.information_data[0] = information.modelNumber
            self.information_data[1] = formatFirmareVersion(information.version)
        elif deviceType == DeviceType.Controller:
            self.information_data[2] = information.modelNumber
            self.information_data[3] = formatFirmareVersion(information.version)
        else:
    ...
    def update_address(self, source, device_address_string):
        if source == DeviceType.Drone:
            ...
            self.address_data[0] = device_address_string
    ...
    def receive_address_data(self,device_address):
        ...
        device_address_string = convertByteArrayToString(device_address_array)
    ...
    def get_address_data(self):
        ...
        self.update_address(DeviceType.Controller, DataType.Address)
        sleep(0.1)
        self.update_address(DeviceType.Controller, self._address)

        return self.address_data

    def open(self, portname=None, updater=False):
        """
        ...
        if state_flight is ModeFlight.Ready and not updater:

            device_info = self.get_information_data()

            if self.information_data[0] == ModelNumber.Drone_12_Drone_P1:
    ...
    def close(self):
        self.disconnect()
        #print(Fore.YELLOW + "Warning: The 'drone.close()' function is deprecated and will be removed in a future release.\nPlease use 'drone.disconnect()' instead." + Style.RESET_ALL)
    ...
    def print_move_values(self):
        ...
        print("Warning: This method is deprecated and will be removed in a future release. Please use 'drone.get_move_values()' instead.")
    ...
    def move_forward(self, distance, units="cm", speed=0.5):
        ...
        if units == "cm":
            ...
            print(Fore.RED+"Error: Invalid unit. Enter a valid unit: 'cm', 'ft', 'in', or 'm'." + Style.RESET_ALL)
            return
    ...
    def move_backward(self, distance, units="cm", speed=0.5):
        ...
        self.send_absolute_position(positionX=x - distance_meters,
                                    ...
                                    rotationalVelocity=0)
        delay = distance_meters / speed
        #print("move backward delay: ", delay)
        time.sleep(delay)
    ...
    def predict_colors(self, color_data):
        try:
            ...
            print(Fore.RED + "Error: A classifier has not been loaded. Call drone.load_classifier() and try again." + Style.RESET_ALL)
            self.disconnect()
            exit()
    ...
    def load_classifier(self, dataset=None, show_graph=False):
        ...
        if dataset is None:  # path to default data inside of cde lib
            ...
            if not os.path.isdir(path):
                print(Fore.RED + "Error: Cannot load classifier. Dataset \"" + dataset + "\" does not exist.")
                ...
                if len(folder) == 0:
                    print(Fore.RED + "Error: Cannot load classifier. Dataset \"" + dataset + "\" is empty.")
        ...
        if len(all_labels) < 3:
            print(Fore.RED + "Error: Dataset must have at least 3 labels to call load_classifier()." + Style.RESET_ALL)
